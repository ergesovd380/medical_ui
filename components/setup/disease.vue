      ...
      >
        <v-card height="256">
          <div class="dialog__title">
            <h3 class="text-center">Вы хотите {{ $t("Delete") }} выбранные база болезней?</h3>
            <div class="mt-10 text-center">
              <v-btn class="dialog__btn" color="var(--secondary-color)" @click="deleteDisease">
                Да
              </v-btn>
        ...
        <v-card height="256">
          <div class="container">
            ...
            <v-text-field
              ...
              class="dialog__input"
              autocomplete="none"
              v-model="editedItem.nameDisase"
              type="text"
            ></v-text-field>
          </div>
          <div class="mt-10 text-center">
            <v-btn class="dialog__btn" color="var(--secondary-color)" :disabled="editedItem.nameDisase.length < 3" @click="addDisease">
              {{ $t("Save") }}
            </v-btn>
...
<script lang="ts">
  export default {
    data(): any {
      return {
        ...
        elements: [
          ...
          {
            id: 2,
            nameDisase: 'Туберкулез'
          },
          {
            id: 3,
            nameDisase: 'Сахарный диабет'
          }
        ],
        editedItem: {
          nameDisase: '',
    ...
    methods: {
      deleteDisease() {
        const disease = this.elements
        const selected = this.selected
        const newDisease = disease.filter(item => !selected.includes(item))
        disease.splice(0, disease.length)
        disease.push(...newDisease)
